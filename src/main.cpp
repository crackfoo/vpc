...
#if defined(NDEBUG)
# error "Voltpotcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4bfe33a5232f440c7f5eb645fe68e72552799159a83f480aea7fec4661a42d55");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Voltpotcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Voltpotcoin Signed Message:\n";
...
{
    // Voltpotcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 168000 blocks, which will occur approximately every 1 years
    nSubsidy >>= (nHeight / 168000); // Voltpotcoin: 840k blocks in ~1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Voltpotcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Voltpotcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Voltpotcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Voltpotcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcaef2bbb113d89b4ea2e18fba8851b3dc7d5f36fe603198396583c4d8001fc57");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "18/August/2020 Voltpot company launches its own crypto currency";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0414e0d42f7798a339caf309b827e6ee0cc7d883a9b974df41f32cfd544ca0a3ac6e89e59c70f06bf84fefc42306ca9b1058374bb226e55fe0077bcf83af57729b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1597756141;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088150923;

        if (fTestNet)
        {
            block.nTime    = 1597756076;
            block.nNonce   = 385749554;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x656aed65b431db4718eca44dbd5c528834635d791dfadb2a9b27fc9cbffd665f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc7, 0xb7, 0xd1 }; // Voltpotcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VoltpotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VoltpotcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VoltpotcoinMiner(CWallet *pwallet)
{
    printf("VoltpotcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("voltpotcoin-miner");
    ...
    try { loop {
        ...
        printf("Running VoltpotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VoltpotcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VoltpotcoinMiner, pwallet));
}
